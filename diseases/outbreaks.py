...
def outbreaks_by_year(year: int):
    return pd.DataFrame(
        gideon_api.query_gideon_api('/diseases/outbreaks',
                                    {'year': year})[_DATA])
...
def latest_outbreaks_by_country(country: ID_OR_STR):
    ...
    return pd.DataFrame(
        gideon_api.query_gideon_api(
            f'/diseases/countries/{country}/latest-outbreaks')[_DATA])
...
def outbreaks_by_disease(disease: ID_OR_STR):
    ...
    return pd.DataFrame(
        gideon_api.query_gideon_api(f'/diseases/{disease}/outbreaks')[_DATA])
...
def endemic_countries_by_disease(disease: ID_OR_STR):
    if isinstance(disease, str):
        disease = lookup_item('diseases', disease)
    return pd.DataFrame(
        gideon_api.query_gideon_api(f'/diseases/{disease}/countries')[_DATA])
...
def endemic_diseases_by_country(country: ID_OR_STR):
    if isinstance(country, str):
        country = lookup_item('countries', country)
    return pd.DataFrame(
        gideon_api.query_gideon_api(f'/diseases/countries/{country}')[_DATA])
